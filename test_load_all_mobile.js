...
const htmlContent = fs.readFileSync(path.join(__dirname, 'mobile_demo.html'), 'utf8');
...
let appJs = fs.readFileSync(path.join(__dirname, 'app_demo.js'), 'utf8');
...
// Strip imports from app_demo.js
...
setTimeout(() => {
  console.log('Testing loading all library items in mobile view...');
  ...
  if (uncaughtErrors.length > 0) {
    console.error(`Total uncaught errors during script run/loading: ${uncaughtErrors.length}`);
    process.exit(1);
  }
  if (failCount > 0) {
    process.exit(1);
